    ...
    def find_networks_without_ratios(self):
        ...
        for net, entries in network_entries.items():
            if len(entries) >= 3:  # Networks with at least 3 entries
                # Check if this network has any ratio entries
                has_ratio = False
                for entry_id in entries:
                    entry = self.graph[entry_id]
                    if 'ratio' in entry.get('en_term', '').lower():
                        has_ratio = True
                        break
...
class GeneratorEngine:
    ...
    def generate_from_gaps(self, gaps):
        """Generate new entries from detected gaps"""
    ...
    def _generate_ratio_for_network(self, gap):
        """Generate a ratio entry for a network"""
        network = gap['network']
        
        # Find matching ratio template
        for num, den, name, net, desc in self.ratio_templates:
            if net == network:
                entry = self._create_ratio_entry(num, den, name, network, desc)
                self.new_entries.append(entry)
                return
        
        # Generic ratio if no match
        entry = self._create_ratio_entry(1, 1, f"{network} ratio", network, f"Ratio for {network}")
        self.new_entries.append(entry)
...
class SelfGrowingSystem:
    ...
    def run_full_cycle(self, seed_mode=False):
        ...
        if seed_mode:
            ...
            new_entries = generator.generate_from_gaps(gaps)
