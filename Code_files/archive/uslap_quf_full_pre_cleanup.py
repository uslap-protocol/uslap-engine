...
def extract_consonants_from_transliteration(source_form: str) -> List[str]:
    ...
    Maps transliteration consonants to their Arabic letter equivalents via reverse lookup."""
    if not source_form:
        return []
...
def source_form_to_arabic_letters(source_form: str) -> List[str]:
    """v2: Convert transliterated source form back to Arabic letter sequence.
...
def batch_validate(lang: str = 'en', show_all: bool = False):
    ...
    for row in rows:
        ...
        # Guard: only for source forms with ≤6 Arabic letters (prevents permutation explosion)
        if not q_result.passed and source_form:
            sf_arabic = source_form_to_arabic_letters(source_form)
