#!/usr/bin/env python3
"""
USLaP SELF-GROWING FOREST SYSTEM - V3 DOMAIN-AWARE ENHANCEMENT
Enhanced with missing domain variant detection for fractal ratio families.
"""
...
# ============================================================================
# COMPONENT 2: PATTERN DETECTOR - ENHANCED WITH DOMAIN VARIANT DETECTION
# ============================================================================

class PatternDetector:
    """Finds gaps and opportunities in the existing data - ENHANCED VERSION"""
    ...
    def _detect_domain_from_term(self, term):
        """Detect which domain a term belongs to"""
        if not term:
            return 'unknown'
        
        term_lower = str(term).lower()
        
        if 'musical' in term_lower or 'fourth' in term_lower or 'sixth' in term_lower or 'interval' in term_lower:
            return 'musical'
        elif 'tempo' in term_lower or 'rhythmic' in term_lower:
            return 'rhythmic'
        elif 'maqam' in term_lower:
            return 'maqam'
        elif 'sacred' in term_lower or 'proportion' in term_lower:
            return 'spiritual/geometric'
        elif 'harmonic' in term_lower or 'string' in term_lower:
            return 'harmonic'
        elif 'temporal' in term_lower or 'cycle' in term_lower:
            return 'temporal'
        elif 'architectural' in term_lower or 'spatial' in term_lower:
            return 'spatial'
        elif 'prayer' in term_lower or 'spiritual' in term_lower:
            return 'spiritual'
        elif 'growth' in term_lower or 'botanical' in term_lower:
            return 'botanical'
        elif 'orbital' in term_lower or 'celestial' in term_lower:
            return 'celestial'
        elif 'mathematical' in term_lower or 'constant' in term_lower:
            return 'numerical'
        else:
            return 'unknown'
    ...
    def detect_all(self):
        """Run all detection methods - ENHANCED with domain variant detection"""
        ...
        self.find_missing_domain_variants()  # NEW: Highest priority
    ...
    def find_missing_domain_variants(self):
        """NEW: Find missing domain variants for ratio families"""
        print("  🔍 Checking for missing domain variants...")
        ...
        for node_id, node in self.graph.items():
            ...
            if num and den:
                ratio = f"{num}/{den}"
                domain = self._detect_domain_from_term(term)
                ...
                ratio_groups[ratio]['entries'].append(node_id)
                ratio_groups[ratio]['domains'].add(domain)
                ratio_groups[ratio]['networks'].add(node.get('network', ''))
        
        if not ratio_groups:
            return
        
        # Define core domains we want each ratio to have
        core_domains = {
            'musical',
            'rhythmic', 
            'spiritual/geometric',
        ...
        # For each ratio, check missing domains
        for ratio, data in ratio_groups.items():
            existing_domains = data['domains']
            missing_domains = core_domains - existing_domains
            
            if missing_domains:
                # Calculate priority: fewer existing domains = higher priority
                domain_coverage = len(existing_domains) / len(core_domains)
                priority = int((1 - domain_coverage) * 100)  # 0-100 scale
                
                # Get a sample network from this ratio's entries
                sample_network = next(iter(data['networks'])) if data['networks'] else 'N08'
                
                for missing_domain in sorted(missing_domains):
                    self.gaps.append({
                        'type': 'missing_domain_variant',
                        'ratio': ratio,
                ...
                if missing_domains:
                    print(f"    Found {len(missing_domains)} missing domains for ratio {ratio}")
...
# ============================================================================
# COMPONENT 3: GENERATOR ENGINE - ENHANCED FOR DOMAIN VARIANTS
# ============================================================================

class GeneratorEngine:
    """Creates new entries from patterns and gaps - ENHANCED FOR DOMAIN VARIANTS"""
    ...
    def __init__(self, reader, detector):
        ...
        # Domain templates for fractal generation
    ...
    def _generate_domain_variant(self, ratio_str, domain_name, domain_desc, network):
        """Generate a specific domain variant for a ratio"""
        # Parse ratio string
        import re
        ...
        # Create entry name based on domain
        if domain_name == 'maqam':
            entry_name = f"maqam proportion ({numerator}/{denominator})"
        else:
            entry_name = f"{domain_desc} {numerator}/{denominator}"
        
        entry = self._create_ratio_entry(
            numerator, denominator,
            entry_name,
            network,
            f"Divine ratio {numerator}/{denominator} manifesting in {domain_name} domain — Q54:49"
        )
    ...
    def _generate_fractal_variants(self, base_num, base_den, network):
        ...
        if base_ratio_str in existing_ratios:
            for domain_name, domain_desc in self.domain_templates:
    ...
    def generate_from_gaps(self, gaps, candidates):
        """Generate new entries from detected gaps and candidates - ENHANCED FOR DOMAIN VARIANTS"""
        print("\n🌱 Generating new entries from gaps...")

        # Process gaps by priority (highest first)
        gaps_sorted = sorted(gaps, key=lambda x: x.get('priority', 0), reverse=True)
        
        gap_count = 0
        processed_ratios = set()  # Track which ratios we've already generated variants for
        
        # First process missing domain variants (highest priority)
        for gap in gaps_sorted:
            if gap['type'] == 'missing_domain_variant':
                self._generate_missing_domain_variant(gap, processed_ratios)
                gap_count += 1
            elif gap['type'] == 'network_needs_ratio':
            ...
            # Limit total gaps processed per cycle
            if gap_count >= 5:
                break
    ...
    def _generate_missing_domain_variant(self, gap, processed_ratios):
        """Generate a missing domain variant for a ratio"""
        ratio = gap['ratio']
        missing_domain = gap['missing_domain']
        network = gap['network']
        
        # Skip if we've already processed this ratio in this cycle
        if ratio in processed_ratios:
            print(f"    Skipping duplicate ratio: {ratio}")
            return
        
        # Find domain description
        domain_desc = "domain variant"
        for domain_name, desc in self.domain_templates:
            if domain_name == missing_domain:
                domain_desc = desc
                break
        
        if missing_domain == 'maqam':
            domain_desc = "maqam proportion"
        
        # Generate the variant
        variant = self._generate_domain_variant(ratio, missing_domain, domain_desc, network)
        if variant:
            self.new_entries.append(variant)
            print(f"    Generated missing domain variant: {variant['EN_TERM']}")
...
# ============================================================================
# COMPONENT 6: MAIN SYSTEM - ENHANCED
# ============================================================================

class SelfGrowingSystem:
    """The main orchestrator that runs all 6 components - ENHANCED VERSION"""
    ...
    def __init__(self, filepath):
        ...
        print("\n" + "="*60)
        print("🌳 USLaP SELF-GROWING FOREST SYSTEM - V3 DOMAIN-AWARE")
        print("="*60)
    ...
    def run_full_cycle(self, seed_mode=False):
        ...
        if seed_mode:
            ...
            # COMPONENT 2: Detect patterns - ENHANCED
            print("\n🔍 COMPONENT 2: Pattern Detector (Enhanced)")
            ...
            # COMPONENT 3: Generate from gaps - ENHANCED
            print("\n🌱 COMPONENT 3: Generator Engine (Enhanced)")
...
def main():
    ...
    if not os.path.exists(filepath):
        ...
        print("  python uslap_forest_v3_domain_aware.py /path/to/your/file.xlsx")
    ...
    try:
        ...
        if choice == '1':
            ...
            if gaps:
                ...
                for i, gap in enumerate(sorted(gaps, key=lambda x: x.get('priority', 0), reverse=True)[:10]):
                    gap_type = gap['type']
